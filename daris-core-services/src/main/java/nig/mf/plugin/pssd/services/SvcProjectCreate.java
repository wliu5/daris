...
public class SvcProjectCreate extends PluginService {
    ...
    public static void addInterfaceDefn(Interface defn) {
        ...
        me = new Interface.Element("user", XmlDocType.DEFAULT,
                "A user of this project. In a federation must be local to this server.", 0, Integer.MAX_VALUE);
        //
        ...
        me = new Interface.Element("role-user", XmlDocType.DEFAULT,
                "Role to become a user of this project. In a federation must be local to this server.", 0,
                Integer.MAX_VALUE);
        me.add(new Interface.Element("name", StringType.DEFAULT, "The name of the role to become a project user.", 0,
                1));
        me.add(new Interface.Element("role",
                new EnumType(new String[] { Project.ADMINISTRATOR_ROLE_NAME, Project.SUBJECT_ADMINISTRATOR_ROLE_NAME,
                        Project.MEMBER_ROLE_NAME, Project.GUEST_ROLE_NAME }),
     ...
     * @throws Throwable
     */
    private void checkUsersExist(XmlDoc.Element args) throws Throwable {

        // Check user members first
        {
            Collection<XmlDoc.Element> ues = args.elements("user");

            if (ues != null) {
                for (XmlDoc.Element me : ues) {
                    ...
                    if (!r.booleanValue("exists")) {
                        throw new Exception("Cannot add project user: the domain (" + domain + ") and/or user ("
                                + user + ") does not exist.");
                    }
                }
            }
        ...
        {
            Collection<XmlDoc.Element> rues = args.elements("role-user");

            if (rues != null) {
                for (XmlDoc.Element me : rues) {
                    ...
                    if (!r.booleanValue("exists")) {
                        throw new Exception(
                                "Cannot add project role-user: the role-user (" + name + ") does not exist.");
                    }
                }
            }
        }
    ...
    public static String createProjectAsset(ServiceExecutor executor, XmlDoc.Element args, String cid)
            throws Throwable {
        String selfMemberRole = null;
        ...
        String parentNS = args.stringValue("namespace", Application.defaultProjectNamespace(executor));
        if (!executor
                .execute("asset.namespace.exists", "<args><namespace>" + parentNS + "</namespace></args>", null, null)
        ...
        if (methods != null || ues != null || rues != null) {
            ...
            if (ues != null) {
                ...
                for (XmlDoc.Element ue : ues) {
                    ...
                    Project.grantProjectRole(executor, cid, userCred, ue.value("data-use"), false);
                }
            }
            ...
            if (rues != null) {
                for (XmlDoc.Element rue : rues) {
                    ...
                    // Set defaults or over-rides for team-member's 'data-use'.
                    ProjectMember.setValidProjectMemberDataUse(projectDataUse, rue);

                    // Grant the hierarchical team role to the role-member
                    Project.grantProjectRole(executor, cid, rue.value("name"), rue.value("role"), false);

                    // Grant data-use role to non p-admin role-members
                    Project.grantProjectRole(executor, cid, rue.value("name"), rue.value("data-use"), false);
                }
            }
    ...
    private static void setProjectNamespaceACLs(ServiceExecutor executor, String projectNS, String projectCid)
            throws Throwable {
        ...
        addProjectNamespaceACL(dm, Role.objectAdminRoleName(),
                new String[] { "administer", "access", "create", "execute", "destroy" },
                new String[] { "access", "create", "modify", "destroy", "licence" },
                new String[] { "access", "modify" });

        // pssd.object.guest
        addProjectNamespaceACL(dm, Role.objectGuestRoleName(), new String[] { "access" }, new String[] { "none" },
                new String[] { "none" });

        // pssd.project.admin
        addProjectNamespaceACL(dm, Project.projectAdministratorRoleName(projectCid),
                new String[] { "administer", "access", "create", "execute", "destroy" },
                new String[] { "access", "create", "modify", "destroy", "licence" },
                new String[] { "access", "modify" });

        // pssd.project.subject.admin
        addProjectNamespaceACL(dm, Project.subjectAdministratorRoleName(projectCid),
        ...
        // pssd.project.guest
        addProjectNamespaceACL(dm, Project.guestRoleName(projectCid), new String[] { "access" },
                new String[] { "access" }, new String[] { "access" });
        executor.execute("asset.namespace.acl.set", dm.root());
    }

    private static void addProjectNamespaceACL(XmlDocMaker dm, String role, String[] namespaceAccesses,
            String[] assetAccesses, String[] assetContentAccesses) throws Throwable {
        dm.push("acl");
