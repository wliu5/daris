...
public class StudyProxyFactory {
	...
	private static StudyProxyHolder createPSSDStudy(ServiceExecutor executor, String studyUID, DataElementMap dem,
			DicomIngestControls ic) throws Throwable {

		if (discardStudy(dem,ic)) {
			return new StudyProxyHolder(null,false);
		}
		...
		if (cms == null || (cms != null && cms.cid() == null)) {
			...
			if (cms == null) {
				...
				return new StudyProxyHolder(null,true);
			}
		} else {
		...
		try {
			...
			if (pssdType.equalsIgnoreCase("subject")) {
				...
				return new StudyProxyHolder(new PSSDStudyProxy(nameSpace, studyUID, cid, null, cms.methodStep(), null, studyMeta,
						domainMetaService, ignoreModality, dropSR), true);
			}
			...
			if (pssdType.equalsIgnoreCase("ex-method")) {
				...
				//
				return  new StudyProxyHolder(new PSSDStudyProxy(nameSpace, studyUID, subjectCid, cid, cms.methodStep(), null, studyMeta,
						domainMetaService, ignoreModality, dropSR), true);
			}
			...
			if (pssdType.equalsIgnoreCase("study")) {
				...
				SubjectHandler.compareSubjectDICOMMetaData(executor, subjectCid, studyMeta, true);
				//
				return  new StudyProxyHolder(new PSSDStudyProxy(nameSpace, studyUID, subjectCid, exMethodCid, cms.methodStep(), cid,
						studyMeta, domainMetaService, ignoreModality, dropSR),true);
			}
	...
	private static Boolean discardStudy (DataElementMap dem, DicomIngestControls ic) throws Throwable {

		// See if we want to discard these data based on meta-data values
		String discardElementName = ic.discardElementName();
		if (discardElementName==null) return false;
		...
		} else if (discardElementName.equals("modality")) {
			String t = dem.stringValue(DicomElements.MODALITY);
			if (t!=null) {
				return (t.equals(discardElementValue));
			}
