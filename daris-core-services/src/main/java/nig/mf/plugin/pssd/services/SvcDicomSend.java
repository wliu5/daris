...
public class SvcDicomSend extends PluginService {
    ...
    static class Logger {
        ...
        Logger(String callingAET, String calledAET, String host, int port) throws Throwable {
            StringBuilder sb = new StringBuilder();
            if (Session.user() != null) {
                sb.append("[").append(Session.user().domain()).append(":").append(Session.user().name()).append("]");
            }
            sb.append("[calling AE:").append(callingAET).append(", called AE:").append(calledAET).append("@")
        ...
        public void logInfo(String msg) {
            ...
            PluginLog.log(LOG_NAME).add(PluginLog.WARNING, sb.append(msg).toString());
        }
        ...
        public void logError(String msg, Throwable t) {
            StringBuilder sb = new StringBuilder(_prefix);
            PluginLog.log(LOG_NAME).add(PluginLog.ERROR, sb.append(msg).toString(), t);
        }

        public void logError(String msg) {
            StringBuilder sb = new StringBuilder(_prefix);
            PluginLog.log(LOG_NAME).add(PluginLog.ERROR, sb.append(msg).toString());
        }
    }
    ...
    public SvcDicomSend() {
        ...
        Interface.Element from = new Interface.Element("calling-ae", XmlDocType.DEFAULT,
                "The details about calling application entity.", 1, 1);
        from.add(new Interface.Element("title", StringType.DEFAULT, "The title of the calling application entity.", 1,
                1));
        _defn.add(from);

        Interface.Element to = new Interface.Element("called-ae", XmlDocType.DEFAULT,
                "The details about called application entity.", 1, 1);
        to.add(new Interface.Element("host", StringType.DEFAULT, "The host address of the called application entity.",
                1, 1));
        to.add(new Interface.Element("port", new IntegerType(0, 65535),
                "The port number of the called application entity.", 1, 1));
        to.add(new Interface.Element("title", StringType.DEFAULT, "The title of the called application entity.", 1, 1));
        _defn.add(to);
        ...
        override.add(ee);
        _defn.add(override);
        _defn.add(new Interface.Element("log", BooleanType.DEFAULT, "On/off logging. Defaults to true.", 0, 1));
    }
    ...
    public void execute(Element args, Inputs inputs, Outputs outputs, XmlWriter w) throws Throwable {
        ...
        if ((ids == null || ids.isEmpty()) && (cids == null || cids.isEmpty()) && where == null) {
            Exception ex = new Exception("Argument 'id', 'cid' or 'where' is required.");
            if (log) {
                PluginLog.log(LOG_NAME).add(PluginLog.ERROR, ex.getMessage(), ex);
            }
            throw ex;
        }
        ...
        final Logger logger = log ? new Logger(callingAETitle, calledAETitle, calledAEHost, calledAEPort) : null;
        /*
         * elements to override
         */
        boolean hasValueRefs = args.elementExists("override/element/value-reference");
        ...
        try {
            for (String datasetAssetId : datasetAssetIds) {
                PluginTask.setCurrentThreadActivity("Extracting dicom files from asset " + datasetAssetId + "...");
                PluginTask.checkIfThreadTaskAborted();
                File assetDir = new File(dir, datasetAssetId);
                assetDir.mkdir();
                if (logger != null) {
                    logger.logInfo("extracting dicom dataset " + datasetAssetId);
                }
                extractAssetContent(executor(), datasetAssetId, assetDir);
                if (override != null && !override.isEmpty()) {
                    PluginTask.setCurrentThreadActivity("Editting dicom files from asset " + datasetAssetId + "...");
                    PluginTask.checkIfThreadTaskAborted();
                    if (hasValueRefs) {
                        if (logger != null) {
                            logger.logInfo("resolve value reference for dicom dataset " + datasetAssetId);
                        }
                        updateOverriddenElementsWithValueRefs(executor(), datasetAssetId, override);
                    }
                    if (logger != null) {
                        logger.logInfo("editing dicom dataset " + datasetAssetId);
                    }
            ...
            new StorageSOPClassSCU(calledAEHost, calledAEPort, calledAETitle, callingAETitle, dicomFiles, 0,
                    new MultipleInstanceTransferStatusHandler() {
                        ...
                        public void updateStatus(int nRemaining, int nCompleted, int nFailed, int nWarning,
                            ...
                            PluginTask.threadTaskCompletedMultipleOf(nCompleted - result[1], total);
                            result[0] = nRemaining;
                            result[1] = nCompleted;
                            result[2] = nFailed;
                            result[3] = nWarning;
                            String activity = "sending " + nCompleted + "/" + (nRemaining + nCompleted)
                                    + " dicom files...";
                            PluginTask.setCurrentThreadActivity(activity);
            ...
            w.add("sent", new String[] { "completed", Integer.toString(nCompleted), "failed", Integer.toString(nFailed),
                    "total", Integer.toString(total) }, nFailed <= 0);
            ...
            if (nCompleted < total || nFailed > 0) {
                activity = "failed. " + nCompleted + " of " + total + " dicom files were sent.";
                PluginTask.setCurrentThreadActivity(activity);
                if (logger != null) {
                    logger.logError(activity);
                }
                throw new DicomException(
                        "Failed to send DICOM files to " + calledAETitle + "@" + calledAEHost + ":" + calledAEPort);
            }
    ...
    private static void ensureAssetExists(ServiceExecutor executor, String id, boolean cid, Logger logger)
            throws Throwable {
        ...
        boolean exists = executor.execute("asset.exists", dm.root()).booleanValue("exists", false);
        if (!exists) {
    ...
    private static void updateOverriddenElementsWithValueRefs(ServiceExecutor executor, String datasetAssetId,
    ...
    private static String getSubjectCid(ServiceExecutor executor, String datasetAssetId) throws Throwable {
        String cid = executor.execute("asset.get", "<args><id>" + datasetAssetId + "</id></args>", null, null)
                .value("asset/cid");
        if (cid != null) {
    ...
    private static String getStudyCid(ServiceExecutor executor, String datasetAssetId) throws Throwable {
        String cid = executor.execute("asset.get", "<args><id>" + datasetAssetId + "</id></args>", null, null)
                .value("asset/cid");
        if (cid != null) {
    ...
    private static XmlDoc.Element getPatientMeta(ServiceExecutor executor, String datasetAssetId) throws Throwable {
        XmlDoc.Element ae = executor.execute("asset.get", "<args><id>" + datasetAssetId + "</id></args>", null, null)
                .element("asset");
        ...
        if (cid != null) {
            ...
            String subjectCid = CiteableIdUtil.getParentId(cid, 3);
            if (subjectCid != null) {
                XmlDoc.Element asset = executor.execute("asset.get", "<args><cid>" + subjectCid + "</cid></args>", null,
                        null);
            ...
            if (studyAssetId != null) {
                XmlDoc.Element studyAE = executor
                        .execute("asset.get", "<args><id>" + studyAssetId + "</id></args>", null, null)
                        .element("asset");
                String patientAssetId = studyAE.value("related[@type='had-by']/to");
                if (patientAssetId != null) {
                    XmlDoc.Element asset = executor.execute("asset.get", "<args><id>" + patientAssetId + "</id></args>",
                            null, null);
                    XmlDoc.Element mdp = asset.element("asset/meta/mf-dicom-patient");
                    mdp = asset.element("asset/meta/mf-dicom-patient-encrypted");
                    return mdp;
    ...
    private static String getPatientId(ServiceExecutor executor, String datasetAssetId) throws Throwable {
    ...
    private static String getPatientName(ServiceExecutor executor, String datasetAssetId) throws Throwable {
        XmlDoc.Element pe = getPatientMeta(executor, datasetAssetId);
        if (pe != null) {
    ...
    private static void addDicomPatient(ServiceExecutor executor, Set<String> datasetIds, XmlDoc.Element patientAsset)
            throws Throwable {
        Collection<String> studyIds = patientAsset.values("related[@type='has']/to");
        for (String studyId : studyIds) {
            XmlDoc.Element studyAsset = executor
                    .execute("asset.get", "<args><id>" + studyId + "</id></args>", null, null).element("asset");
            addDicomStudy(executor, datasetIds, studyAsset);
        }
    }

    private static void addDicomStudy(ServiceExecutor executor, Set<String> datasetIds, XmlDoc.Element studyAsset)
            throws Throwable {
        if (studyAsset.elementExists("related[@type='contains']/to")) {
            datasetIds.addAll(studyAsset.values("related[@type='contains']/to"));
        }
    }

    private static void addPssdDicomStudy(ServiceExecutor executor, Set<String> datasetIds, String studyCid)
            throws Throwable {
        Collection<String> ids = executor.execute("asset.query",
    ...
    private static void addPssdDicomSubject(ServiceExecutor executor, Set<String> datasetIds, String subjectCid)
            throws Throwable {
        Collection<String> ids = executor.execute("asset.query",
        ...
        if (ees != null) {
            ...
            for (XmlDoc.Element ee : ees) {
                ...
                if (name == null && tagStr == null) {
                    throw new Exception("Either override/element/@tag or override/element/@name must be specified.");
                }
                if (name != null && tagStr != null) {
                    throw new Exception(
                ...
                AttributeTag tag = new AttributeTag(Integer.parseInt(group, 16), Integer.parseInt(element, 16));
                if (ee.booleanValue("@anonymize", false)) {
                    map.put(tag, " ");
                } else {

                    String value = ee.value("value");
                    ValueReference valueRef = ValueReference.fromString(ee.value("value-reference"));
                    if (value != null) {
                        map.put(tag, value);
                    } else if (valueRef != null) {
                        map.put(tag, valueRef);
                    } else {
                        throw new Exception("override/element[@tag='" + tagStr + "']/value or override/element[@tag='"
                                + tagStr + "']/value-reference must be specified.");
                    }
                }
            }
            if (!map.isEmpty()) {
    ...
    private static void extractAssetContent(ServiceExecutor executor, String assetId, File dir) throws Throwable {
        ...
        if (ArchiveRegistry.isAnArchive(mtype)) {
            ...
            File of = new File(dir, cext == null ? assetId : (assetId + "." + cext));
    ...
    private static void editDicomFile(File f, Map<AttributeTag, Object> overriddenValues, String fromAET)
            throws Throwable {
        ...
        if (mediaStorageSOPClassUID != null && mediaStorageSOPInstanceUID != null) {
            ...
            FileMetaInformation.addFileMetaInformation(list, TransferSyntax.ExplicitVRLittleEndian, fromAET);
        }
    ...
    private static void editDicomFiles(File dir, Map<AttributeTag, Object> overriddenValues, String fromAET)
            throws Throwable {
    ...
    private static SetOfDicomFiles listDicomFilesRecursively(File dir) throws Throwable {
        List<File> files = new Vector<File>();
        listFilesRecursively(dir, files);
    ...
    private static void addByAssetMeta(ServiceExecutor executor, XmlDoc.Element ae, Set<String> datasetAssetIds)
            throws Throwable {
        if (!ae.elementExists("meta/mf-dicom-patient") && !ae.elementExists("meta/mf-dicom-patient-encrypted")
                && !ae.elementExists("meta/mf-dicom-study") && !ae.elementExists("meta/mf-dicom-series")
