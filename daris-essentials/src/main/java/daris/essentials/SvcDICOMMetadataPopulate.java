...
public class SvcDICOMMetadataPopulate extends PluginService {
	...
	public SvcDICOMMetadataPopulate() {
		...
		_defn.add(new Interface.Element("list-only", BooleanType.DEFAULT,
				"Set to true to just list the values. Default is false.", 0, 1));
	}
	...
	public void execute(Element args, Inputs inputs, Outputs outputs, XmlWriter w) throws Throwable {
		...
		if (hasIdArg && !hasCidArg && !hasWhereArg) {
			...
			populate(ids, false, idx, tags, ignoreErrors, ifExists, docTag, listOnly, w);
		} else if (!hasIdArg && hasCidArg && !hasWhereArg) {
			// cids
			Collection<String> cids = args.values("cid");
			populate(cids, true, idx, tags, ignoreErrors, ifExists, docTag, listOnly, w);
		} else if (!hasIdArg && !hasCidArg && hasWhereArg) {
			// where
	...
	private void populate(Collection<String> ids, boolean citeable, int idx, Set<String> tags, boolean ignoreErrors,
			IfExists ifExists, String docTag, boolean listOnly, XmlWriter w) throws Throwable {
